...
{   
    /* This routine runs shifted inverse power iteration algorithm to find largest eigenvalue
    input = x(0) = random vector,
    max_itr=5,
    tolerence=1e-6,
    matrix A = random real & symmetric matrix of size n x n
...
double rayleigh_quotient_iteration(const matrix* A, const int max_iter, const double tol)
{
    ...
    /* This routine runs Rayleigh quotient iteration algorithm to find largest eigenvalue
    input = x(0) = random vector,
    max_itr=5,
    tolerence=1e-6,
    matrix A = random real & symmetric matrix of size n x n
